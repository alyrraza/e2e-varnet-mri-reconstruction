...
LR = 3e-4
TOTAL_EPOCHS = 50
BATCH_SIZE = 4          # <-- 1 se 4 kiya, VRAM use karne ke liye
GRAD_ACCUM_STEPS = 2    # <-- 8 se 2 kiya (effective batch same rahe: 4*2=8, pehle 1*8=8)
...
# num_workers 4 -> 8 (zyada CPU cores available hain Vast.ai pe)
train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=8, persistent_workers=False)
val_loader = DataLoader(val_dataset, batch_size=BATCH_SIZE, shuffle=False, num_workers=8, persistent_workers=False)
