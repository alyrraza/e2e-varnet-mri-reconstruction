...
SENS_CHANS = 8
LR = 3e-4
TOTAL_EPOCHS = 30   # reduced from 50 - T=6 pattern showed convergence ~epoch 20-22
GRAD_ACCUM_STEPS = 8
...
print(f"T=8 model parameters: {total_params:,}", flush=True)
...
for epoch in range(TOTAL_EPOCHS):
    ...
    pbar = tqdm(train_loader, desc=f"T=8 Epoch {epoch+1}/{TOTAL_EPOCHS}", leave=True)
    ...
    for i, batch in enumerate(pbar):
        ...
        if current_time - last_print_time >= PRINT_EVERY_SECONDS:
            print(f"[STATUS] T=8 Epoch {epoch+1}/{TOTAL_EPOCHS}, Batch {i+1}/{len(train_loader)}, Running avg loss: {epoch_loss/(i+1):.4f}", flush=True)
    ...
    print(f"\n=== T=8 Epoch {epoch+1} complete (took {epoch_duration/60:.1f} min) ===", flush=True)
    ...
    }, f"/workspace/t8_checkpoint_epoch_{epoch+1}.pt")

    if avg_val_ssim > best_val_ssim:
        best_val_ssim = avg_val_ssim
        torch.save({'epoch': epoch, 'model_state_dict': model.state_dict(), 'val_ssim': avg_val_ssim, 'num_cascades': NUM_CASCADES}, "/workspace/t8_best_model.pt")
        print(f"[NEW BEST] Saved t8_best_model.pt (val SSIM: {avg_val_ssim:.4f})\n", flush=True)

print("=== T=8 TRAINING COMPLETE ===", flush=True)
